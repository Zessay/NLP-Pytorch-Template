...
class AlbertMSN(BaseModel):
    ...
    def get_default_params(self) -> ParamTable:
        ...
        params.add(Param(
            ...
            desc="The padding idx of short text."
        ))
    ...
    def get_matching_map(self, U_embed, R_embed, U_mask, R_mask):
        ...
        # M = torch.stack([M1, M2, M5, M6], dim=1)
        return M
    ...
    def forward(self, inputs):
        ...
        resp_mask = get_pad_mask(response, self._params['padding_idx']).squeeze(dim=-2)
        ...
        # 2. 经过几层卷积层提取特征
        ## [B*Nu, linear_out]
        V = self.UR_matching(multi_context, R_emb,
                              U_mask=uttrs_mask,
                             R_mask = resp_mask)
